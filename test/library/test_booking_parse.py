...
CANCEL_BTN = '<a class="btn btn-lg btn-block btn-primary">取消预约</a>'
...
class LibraryBookingTest(unittest.TestCase):
    ...
    def test_webvpn_entity_quoted_actions(self):
        ...
        """
        html = card_html("北楼二层外文库（东）", "D004", "待入馆", CANCEL_BTN) + """
    ...
    def test_get_seat_layout_parses_coords_and_status(self):
        """可视化选座：坐标矩形 + 状态码；cancel 键保留为返回按钮矩形。"""
        ...
        self.library.session.get = lambda *a, **k: make_response(payload)
        positions, status = self.library.get_seat_layout("north2east")
        self.assertEqual(positions["A101"], (10.5, 20.0, 30.0, 40.0))
        self.assertEqual(status["A101"], 2)
        self.assertEqual(status["A102"], 0)
        self.assertEqual(status["A103"], -1)
        # cancel 是图上的返回按钮矩形，保留（status=-2）
        self.assertEqual(positions["cancel"], (999.0, 999.0, 1.0, 1.0))
        self.assertEqual(status["cancel"], -2)
        self.assertNotIn("", positions)
    ...
    def test_book_seat_success_lands_on_my_page(self):
        html = card_html("北楼二层外文库（东）", "A101", "已预约", CANCEL_BTN) + """
        <script>showConfirmModal('确认', 'cancel', '88')</script>
        """
